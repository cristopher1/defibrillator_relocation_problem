...
/*implementacion de los metodos de la clase ResultadoCD*/

ResultadoCD::ResultadoCD() {
  this -> presupuesto = 0.0;
  this -> radio = 0;
  this -> nEventos = 0;
  this -> nAedsIniciales = 0;
  this -> enfoque = NULL;
  this -> coordsX = posicion();
  this -> coordsY = posicion();
  this -> aedsIniciales = posicion();
}

ResultadoCD::ResultadoCD(const float presupuesto, const unsigned int radio, const unsigned int nEventos,
      const unsigned int nAedsIniciales, const char *enfoque, const posicion coordsX,
      const posicion coordsY, const posicion aedsIniciales) {
        this -> presupuesto = presupuesto;
        this -> radio = radio;
        this -> nEventos = nEventos;
        this -> nAedsIniciales = nAedsIniciales;
        this -> enfoque = enfoque;
        this -> coordsX = coordsX;
        this -> coordsY = coordsY;
        this -> aedsIniciales = aedsIniciales;
      }

float ResultadoCD::getPresupuesto() {
  return this -> presupuesto;
}

unsigned int ResultadoCD::getRadio() {
  return this -> radio;
}

unsigned int ResultadoCD::getNEventos() {
  return this -> nEventos;
}

unsigned int ResultadoCD::getNAedsIniciales() {
  return this -> nAedsIniciales;
}

const char* ResultadoCD::getEnfoque() {
  return this -> enfoque;
}

posicion ResultadoCD::getCoordsX() {
  return this -> coordsX;
}

posicion ResultadoCD::getCoordsY() {
  return this -> coordsY;
}

posicion ResultadoCD::getAedsIniciales() {
  return this -> aedsIniciales;
}
...
ResultadoCT::ResultadoCT() {
  this -> cobertura = 0;
  this -> eventosCubiertos = set<coordenadas>();
}

ResultadoCT::ResultadoCT(const unsigned long cobertura, const set<coordenadas> eventosCubiertos) {
...
set<coordenadas> ResultadoCT::getEventosCubiertos() {
...
ResultadoFEv::ResultadoFEv(const float costo, const unsigned long cobertura,
  const set<coordenadas> eventosCubiertos) {
  this -> costo = costo;
  this -> cobertura = cobertura;
  this -> eventosCubiertos = eventosCubiertos;
}
...
set<coordenadas> ResultadoFEv::getEventosCubiertos() {
  return this -> eventosCubiertos;
}
...
ResultadoCD cargar_datos(const char *nombre_archivo) {
  ...
  cin >> n_eventos >> presupuesto >> radio;
  for(unsigned int c = 0; c < n_eventos; c++) {
    cin >> coord_x >> coord_y >> aed_inicial;
  ...
  return ResultadoCD(presupuesto, radio, n_eventos, n_aeds_iniciales, enfoque, coords_x, coords_y,
    aeds_iniciales);
}
...
ResultadoCT cobertura_total_inicial(vector<set<coordenadas>> coberturas, posicion solucion_candidata) {
  set<coordenadas> eventos_cubiertos;
...
ResultadoCT cobertura_total(const vector<set<coordenadas>> coberturas,
  const set<coordenadas> eventos_cubiertos, const posicion solucion_candidata,
  const unsigned int posicion, const bool agregado) {
  set<coordenadas> nuevos_eventos_cubiertos = eventos_cubiertos;
  /*
...
ResultadoFEv f_ev(const vector<set<coordenadas>> coberturas, const posicion aeds_iniciales,
  const posicion solucion_candidata, const set<coordenadas> eventos_cubiertos,
...
vector<set<coordenadas>> obtener_coberturas(const vector<unsigned long> coords_x,
  const vector<unsigned long> coords_y, const unsigned int radio) {
  vector<set<coordenadas>> coberturas;
  /*
  ...
  for(auto iter_x_act = coords_x.begin(), iter_y_act = coords_y.begin();
    ...
    for(auto iter_x_sig = coords_x.begin(), iter_y_sig = coords_y.begin();
      ...
      if(radio_act <= radio) {
        cobertura.insert(make_pair(*iter_x_sig, *iter_y_sig));
      }
    }
...
void info_aeds_fijo(const posicion aeds_iniciales, const posicion mejor_solucion,
  ...
  cout << "Cantidad de AEDs agregados: " << n_aeds_agregados << endl;
  cout << "Posiciones de los AEDs: " << endl;
  for(unsigned int pos = 0; pos < mejor_solucion.size(); pos++) {
    if(mejor_solucion[pos]) {
      cout << "coordenada X: " << coords_x[pos] << " coordenada Y: " << coords_y[pos] << endl;
    }
  }
}
...
void info_aeds_flexible(const posicion aeds_iniciales, const posicion mejor_solucion,
  ...
  cout << "Cantidad de AEDs agregados: " << n_aeds_agregados << endl;
  cout << "Cantidad de AEDs movidos: " << n_aeds_movidos << endl;
  for(unsigned int pos = 0; pos < mejor_solucion.size(); pos++) {
    if(mejor_solucion[pos]) {
      cout << "coordenada X: " << coords_x[pos] << " coordenada Y: " << coords_y[pos] << endl;
    }
...
ResultadoHCMM hc_mm(vector<set<coordenadas>> coberturas, posicion aeds_iniciales,
  ...
  */
  ...
  set<coordenadas> mejor_eventos_cubiertos_act = resultado_cobertura.getEventosCubiertos();
  set<coordenadas> mejor_eventos_cubiertos = mejor_eventos_cubiertos_act;
...
void resolver(const int n_eventos, const int radio, const float presupuesto, const int n_restart,
  const unsigned int n_aeds_iniciales, const posicion aeds_iniciales, const posicion coords_x,
  const posicion coords_y, costo const cost, generar_solucion const solucion, imprimir const info) {

  ResultadoHCMM mejor;
  vector<set<coordenadas>> coberturas = obtener_coberturas(coords_x, coords_y, radio);
  ...
  cout << "Cobertura: " << mejor.getCobertura() << endl;
  cout << "Porcentaje de cobertura: " << fixed << setprecision(2) << (mejor.getCobertura() / (float) n_eventos) * 100 << "%" << endl;
  cout << "Presupuesto sobrante: " << presupuesto - mejor.getCosto() << endl;
